...
const Dashboard = () => {
  ...
  const NavItem = ({ icon, text, active, onClick }) => (
    <div 
      onClick={onClick}
      className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all duration-300 font-medium
    ...
    <div className="min-h-screen bg-[#f4f7fe] flex w-full font-sans text-gray-800">
      ...
      <div className="hidden lg:flex w-72 bg-white flex-col shadow-xl fixed h-full z-10">
        ...
        <div className="flex flex-col gap-2 p-4 flex-1 mt-4">
          <NavItem icon={<FiHome />} text="Dashboard" active={activeTab === 'Dashboard'} onClick={() => handleNavClick('Dashboard')} />
          <NavItem icon={<FiBox />} text="Products" active={activeTab === 'Products'} onClick={() => handleNavClick('Products')} />
          <NavItem icon={<FiUsers />} text="Customers" active={activeTab === 'Customers'} onClick={() => handleNavClick('Customers')} />
          <NavItem icon={<FiActivity />} text="Analytics" active={activeTab === 'Analytics'} onClick={() => handleNavClick('Analytics')} />
          <NavItem icon={<FiSettings />} text="Settings" active={activeTab === 'Settings'} onClick={() => handleNavClick('Settings')} />
        </div>
        ...
        <div className="hidden lg:flex justify-between items-center p-8 bg-[#f4f7fe]/80 backdrop-blur-md sticky top-0 z-10">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              {activeTab === 'Dashboard' ? "Welcome back! Here's your store's fresh status." : `Manage your ${activeTab.toLowerCase()} here.`}
            </p>
          </div>
          <button 
            onClick={() => {
          ...
          {activeTab === 'Dashboard' ? (
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
              <div className="w-20 h-20 bg-blue-50 text-blue-500 rounded-full flex items-center justify-center text-4xl mb-4">
                {activeTab === 'Products' && <FiBox />}
                {activeTab === 'Customers' && <FiUsers />}
                {activeTab === 'Analytics' && <FiActivity />}
                {activeTab === 'Settings' && <FiSettings />}
              </div>
              <h3 className="text-2xl font-bold text-gray-800 mb-2">{activeTab} Module</h3>
              <p className="text-gray-500 max-w-md mx-auto">This section is currently under development. Check back later for detailed {activeTab.toLowerCase()} management features.</p>
            </div>
          )}
